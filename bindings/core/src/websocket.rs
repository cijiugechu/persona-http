...
use wreq::{
    ...
    ws::{self, WebSocketResponse, message::{self, CloseCode, CloseFrame, Utf8Bytes}},
};
...
impl Message {
    ...
    pub fn close(&self) -> Option<(u16, Option<&str>)> {
        match &self.0 {
            message::Message::Close(Some(frame)) => Some((
                u16::from(frame.code.clone()),
                Some(frame.reason.as_ref()),
            )),
            _ => None,
        }
    }
...
enum Command {
    ...
    Recv(Option<Duration>, oneshot::Sender<Result<Option<Message>, Error>>),
    Close(Option<u16>, Option<String>, oneshot::Sender<Result<(), Error>>),
}
...
async fn recv(cmd: UnboundedSender<Command>, timeout: Option<Duration>) -> Result<Option<Message>, Error> {
